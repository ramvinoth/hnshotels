...
<div id="choisir_restau" data-role="page" data-add-back-btn="true">
	...
	<?php
		if(isset($_GET['category'])){
			...
			$drinks ="";
			$sql= "SELECT * from products_111_cht WHERE category='$category'";
			$results = mysqli_query($mysqli_conn, $sql)or die ('Unable to execute query. '. mysqli_error($mysqli_conn));
			while($row = mysqli_fetch_array($results, MYSQLI_ASSOC))
			{
				$desc = "";
				$desc = explode(".", $row['items']);
				foreach ($mix as $value) {						 
					$desc .=$value.'<br/>';
				}
				$drinks .= '<li><a href="#'.$row['p_id'].'" data-rel="popup" data-position-to="window" data-role="button" data-inline="true" data-transition="pop"> <h5>'.$row['p_name'].'<i class="fa fa-inr fr">'.$row['p_price'].'</i></h5></a> </li>';
				...
				$drinks .= '
							<div data-role="popup" id="'.$row['p_id'].'">
								...
								<div class="item-box" data-role="content">
									<label for="slider2b">Quantity:</label>
									<input class="qty" type="range" name="qty" id="slider2b" value="1" min="1" max="5" data-highlight="true">
									<hr>
										<label for="addon_'.$row['p_id'].'" class="select">Addons (Mix)</label>
				...
				$drinks .='				</select>
									...
									<button onclick=\'$("#cancel").trigger( "click" );\' href="dialog/index.html" data-role="toast" data-toast-message="Your Drink Added to Cart" data-ajax=\'false\' data-icon="false" data-rel="back" data-inline="true" class="ui-btn ui-btn-primary"><i class=\'zmdi zmdi-check\'></i>&nbsp;Add To cart</button>
...
<script src="//ajax.googleapis.com/ajax/libs/jquery/2.1.4/jquery.min.js"></script><script type="text/javascript">
	...
	$(".item-box button").click(function(e){ //user click "add to cart" button
		...
		console.log("content : "+iqty+"  "+addon);
		...
		$.ajax({ //make ajax request to cart_process.php
			...
			data: { quantity: iqty, product_code: icode, addon_name:addon, type:"foods"}
