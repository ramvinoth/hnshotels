...
<body>
                                    ...
                                    <tbody>
										...
										while($row = $results->fetch_assoc()) {
											if($row["status"] == 'On-Process')
												...
												$disp.= '<td><a data-toggle="modal" data-target="#deliveryBoy" style="cursor:pointer;"	onclick="update(\''.$row["order_id"].'\')">'.$row["order_id"].'</a></td>';
												$disp.= '<td>'.$row["store_id"].'</td>';
												$disp.= '<td>'.$row["amount"].'</td>';
												$disp.= '<td>'.$row["type"].'</td>';
												$disp.= '<td>'.$row["payment"].'</td>';
	...
	function viewBill()
	{
		if(emp_no != -1){
			...
			window.location.assign("bill.php?emp_name="+arr[1]+"&emp_no="+arr[0]+"&order_id="+id);
		}
		else	
